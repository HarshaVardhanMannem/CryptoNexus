...
import { OnChainReport, Subtask } from '../types/agent.types';
...
export class OnChainAnalysisAgent {
     ...
     */
    async execute(subtask: Subtask): Promise<OnChainReport> {
     ...
     */
    private async fetchExchangeFlows(symbol: string): Promise<ExchangeFlow> {
        try {
     ...
     */
    private async trackWhaleMovements(symbol: string): Promise<WhaleTransaction[]> {
        try {
     ...
     */
    private async getNetworkMetrics(symbol: string): Promise<any> {
        try {
...
`;
     ...
     */
    private calculateNetDirection(whaleActivity: WhaleTransaction[]): 'to_exchanges' | 'from_exchanges' | 'neutral' {
        ...
        if (deposits > withdrawals + 1) return 'to_exchanges';
        if (withdrawals > deposits + 1) return 'from_exchanges';
        return 'neutral';
    }
     ...
     */
    private generateFallbackAnalysis(exchangeFlows: ExchangeFlow, whaleActivity: WhaleTransaction[]): string {

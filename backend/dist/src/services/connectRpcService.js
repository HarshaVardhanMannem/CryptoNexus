...
exports.ConnectRpcService = void 0;
const connect_express_1 = require("@bufbuild/connect-express");
const scraper_1 = require("../playwright/scraper");
const ticker_connect_1 = require("../../gen/proto/ticker_connect");
const ticker_pb_1 = require("../../gen/proto/ticker_pb");
class ConnectRpcService {
    constructor() {
        this.subscriptions = new Map(); // symbol -> clientIds
        this.scrapers = new Map(); // symbol -> scraper
        this.priceCallbacks = new Map(); // symbol -> callback
        this.clientStreams = new Map(); // clientId -> stream context
        this.clientSymbols = new Map(); // clientId -> symbol
        this.lastPriceBySymbol = new Map(); // symbol -> last PriceUpdate
        this.cleanupTimers = new Map(); // symbol -> timer
    ...
    createMiddleware() {
        return (0, connect_express_1.expressConnectMiddleware)({
            routes: (router) => {
                router.service(ticker_connect_1.TickerService, {
                    // Server streaming for real-time price updates
                    subscribeTicker: async function* (request, context) {
                        const service = this;
                    ...
                    unsubscribeTicker: async (request, context) => {
                        ...
                        return new ticker_pb_1.PriceUpdate({
                            symbol,
                            price: 0,
    ...
    async *createPriceStream(symbol, clientId) {
        ...
        try {
            ...
            const priceQueue = [];
            let resolveNextPrice = null;
            // Set up callback for price updates
            const priceCallback = (price, isoTimestamp) => {
                const priceUpdate = new ticker_pb_1.PriceUpdate({
                    symbol,
                    price,
                    isoTimestamp
                });
    ...
    async startScraping(symbol) {
        ...
        try {
            ...
            const priceCallback = (price, isoTimestamp) => {
                ...
                const cachedUpdate = new ticker_pb_1.PriceUpdate({ symbol: this.normalizeSymbol(symbol), price, isoTimestamp });
...
exports.ConnectRpcService = ConnectRpcService;

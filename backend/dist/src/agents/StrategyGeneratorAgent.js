...
exports.StrategyGeneratorAgent = void 0;
...
class StrategyGeneratorAgent {
     ...
     */
    async execute(subtask, taskState) {
        ...
        const strategy = await this.generateStrategyWithAI(symbol, marketData, sentimentData, onChainData, description);
     ...
     */
    extractMarketData(taskState) {
        const marketSubtasks = taskState.subtasks.filter(st => st.agentType === 'market_monitor' && st.status === 'completed' && st.result);
        if (marketSubtasks.length === 0)
            return null;
     ...
     */
    extractSentimentData(taskState) {
        const sentimentSubtasks = taskState.subtasks.filter(st => st.agentType === 'news_sentiment' && st.status === 'completed' && st.result);
        if (sentimentSubtasks.length === 0)
            return null;
     ...
     */
    extractOnChainData(taskState) {
        const onChainSubtasks = taskState.subtasks.filter(st => st.agentType === 'onchain_analysis' && st.status === 'completed' && st.result);
        if (onChainSubtasks.length === 0)
            return null;
     ...
     */
    async generateStrategyWithAI(symbol, marketData, sentimentData, onChainData, taskDescription) {
        const prompt = `
...
`;
        try {
            ...
            if (jsonMatch) {
                ...
                return {
                    symbol,
                    recommendation: (strategyData.recommendation || 'HOLD'),
                    ...
                    timeHorizon: (strategyData.timeHorizon || 'MEDIUM'),
     ...
     */
    generateFallbackStrategy(symbol, marketData, sentimentData, onChainData) {
        ...
        if (sentiment === 'bullish')
            ...
            score -= 1;
        if (exchangeFlow === 'outflow')
            score += 1; // Accumulation
        else if (exchangeFlow === 'inflow')
            score -= 1; // Distribution
        if (marketData?.indicators?.rsi) {
            if (marketData.indicators.rsi < 30)
                score += 1; // Oversold
            else if (marketData.indicators.rsi > 70)
                score -= 1; // Overbought
        }
...
exports.StrategyGeneratorAgent = StrategyGeneratorAgent;
